...
import { IOS, ANDROID, VKCOM, Platform } from '../../lib/platform';
...
class View extends Component<ViewProps & DOMProps, ViewState> {
  ...
  componentDidUpdate(prevProps: ViewProps, prevState: ViewState) {
    ...
    const isNewAnimate = !prevState.animated && this.state.animated;
    const isNestedAnimate =
      prevState.animated && this.state.animated &&
      prevState.activePanel === null && this.state.activePanel === null;

    // Начался переход
    if (isNewAnimate || isNestedAnimate) {
      ...
      if (isNestedAnimate) {
        const oldPanelElement = this.pickPanel(prevState.prevPanel);
        oldPanelElement.removeEventListener(animationEvent.name, this.transitionEndHandler);
      }

      if (this.state.isBack) {
        prevPanelElement.removeEventListener(animationEvent.name, this.transitionEndHandler);
        prevPanelElement.addEventListener(animationEvent.name, this.transitionEndHandler);
      } else {
        nextPanelElement.removeEventListener(animationEvent.name, this.transitionEndHandler);
        nextPanelElement.addEventListener(animationEvent.name, this.transitionEndHandler);
      }
    }
  ...
  render() {
    const {
      ...
      activePanel: _, splitCol, configProvider, history, id,
      onTransition, onSwipeBack, onSwipeBackStart, onSwipeBackCancel,
      window, document,
      ...restProps
    } = this.props;
    const { prevPanel, nextPanel, activePanel, stalePanels, swipeBackPrevPanel, swipeBackNextPanel, swipeBackResult } = this.state;
    ...
    const panels = this.panels.filter((panel: React.ReactElement) => {
      ...
      return panelId === activePanel ||
        panelId === prevPanel ||
        panelId === nextPanel ||
        panelId === swipeBackPrevPanel ||
        panelId === swipeBackNextPanel ||
        stalePanels.includes(panelId);
    });

    const disableAnimation = this.shouldDisableTransitionMotion();

    const isBack = this.state.isBack === true;
    const isForward = this.state.isBack === false;
    ...
    return (
      ...
      >
        <div vkuiClass="View__panels">
          {panels.map((panel: React.ReactElement, index) => {
            const panelId = panel.props.id;
            const isStale = stalePanels.includes(panelId);

            return (
              <div
                vkuiClass={classNames('View__panel', {
                  'View__panel--active': panelId === activePanel,
                  'View__panel--prev': (isForward && index === 0) || (isBack && ((panelId === prevPanel) || isStale)),
                  'View__panel--next': ((isForward && index !== 0) || (isBack && isStale)) && ((panelId === nextPanel) || (isForward && isStale)),
